...
so i changed career paths to build new tools designed specifically for data scientists to deploy models. my current role is to develop an open source package called vetiver, which is an mlops framework to help data scientists do model operations better in R and Python
:::
...
|R                                                        | Python                                                 |
|---------------------------------------------------------|--------------------------------------------------------|
| `vetiver_model()` `new_vetiver_model()`                 | `VetiverModel()`                                       |
| `vetiver_pin_write()` `vetiver_pin_read()`              | `vetiver_pin_write()` `VetiverModel.from_pin()`        |
| `vetiver_api()` `vetiver_pr_post()`                     | `VetiverAPI()` `VetiverAPI.vetiver_post()`             |
| `vetiver_write_plumber()`                               | `write_app()`                                          |
| `vetiver_compute_metrics()`                             | `compute_metrics()`                                    |
| `vetiver_pin_metrics()`                                 | `pin_metrics()`                                        |
...
now, these will require a few more arguments for credentials
:::
...
production can mean a lot of different things. in fact, you might have deployed a model without even knowing it
:::
...
::: notes
the IDEA of data science workflows are directly opposite of software engineering best practices. where software engineers are interested in robust applications and strong testing and absolute certainty that something will behave as expected, data science requires agility and experimental behavior
:::
...
::: {.incremental}
- REST APIs
  - run on a local laptop
  - run inside a container, such as Docker
:::

::: notes


REST APIs (Representational state transfer application programming interface) are uniform interface between physically separate components. standardized way for machines to communicate to each other.

&&&

tools like vetiver use pre-set interfaces such as open api specifications. these standards for how software programs communicate with each other help developers have to write less code to make different software programs work together seamlessly.
...
this makes the software engineering part of me very very happy, since decoupling the 
...
::: {.notes}
besides me thinking this is kinda funny, this actually brings us to another great point
...
:::{.notes}
will create a file named app.py with all the information needed

just a python script. it will run out of the box (pending you give it the right credentials)
...
::: {.notes}
model drift when your model's performance metrics start decaying. this is SO IMPORTANT TO TRACK. 
...
::: {.notes}
common failure is storing data that might overlap in date
...
-   Talk from rstudio::conf() 2022 on [Demystifying MLOps](https://www.rstudio.com/conference/2022/talks/demystifying-mlops/) 🍪
